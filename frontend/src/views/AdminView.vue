  ...
  <div class="d-flex-column align-items-start">
    <div id="title" class="sticky-top p-3" style="background-color: white">
      ...
      <div class="form-group row pt-2 pb-3">
        <div class="col-auto d-flex align-items-center">
          <select class="form-select" v-model="selectWoche" @change="_fetchPlanByWeek(selectWoche)">
          ...
          <i v-if="successPlanUpdate" class="fa fa-check txt-moos" style="margin-left: 1rem"></i>
        </div>
      </div>
    </div>
    ...
    <div>
      <div class="d-flex flex-wrap justify-content-start">
        <div
          v-for="(tag, index) in wochenplan"
          class="d-flex flex-grow-1 flex-sm-grow-0 flex-md-grow-0"
        >
          <div
            class="card m-3 flex-grow-1"
            style="width: 200px"
            v-bind:style="[
          ...
          >
            ...
            >
              <p class="card-text m-0">
                <input
                  type="checkbox"
                  :name="(tagOffen[index] = tag.offen)"
                  v-model="tag.offen"
                  :disabled="new Date(tag.datum + 'T16:30:00') < new Date()"
                  @change="updatePlan(tag.id, index)"
                />
                <b>Öffnungszeiten</b>
              </p>
              <div v-if="tag.offen">
                ...
                <div v-if="tag.wald" class="input-group input-group-sm pt-2">
                  ...
                  <input
                    type="time"
                    class="form-control"
                    :name="(tagAbfahrt[index] = tag.abfahrt)"
                    v-model="tag.abfahrt"
                    required
                    @blur="updatePlan(tag.id, index)"
                    :disabled="new Date(tag.datum + 'T16:30:00') < new Date()"
              ...
              <div class="input-group input-group-sm pb-1">
                <input
                  type="text"
                  class="form-control"
                  :name="(tagKommentar[index] = tag.kommentar)"
                  v-model="tag.kommentar"
                  @blur="updatePlan(tag.id, index)"
                  :disabled="new Date(tag.datum + 'T16:30:00') < new Date()"
                />
              </div>
    ...
    <div class="d-flex justify-content-start">
      <a class="btn btn-sm btn-anmelden mx-3" v-if="keycloak.authenticated" @click="logout"
        >Abmelden</a
      >
    </div>
